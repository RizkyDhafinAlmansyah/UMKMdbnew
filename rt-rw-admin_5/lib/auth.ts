...
// Update interface User untuk menambahkan field baru
...
// Update AuthContextType untuk menambahkan fungsi changePassword
...
// Update admin users untuk setiap RW
const ADMIN_USERS = [
  {
    id: "admin-rw01",
    username: "admin",
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    // Check admin login dengan RW selection
    if (username === "admin" && password === DEFAULT_ADMIN_PASSWORD) {
      // Jika masih menggunakan password default, tampilkan pilihan RW
      const rwChoice = prompt("Pilih RW Anda (01, 02, 03, 04, 05):")
      if (!rwChoice || !["01", "02", "03", "04", "05"].includes(rwChoice)) {
        alert("RW tidak valid. Pilih antara 01-05.")
        return false
      }

      const adminUser = ADMIN_USERS.find((admin) => admin.rw === rwChoice)
      if (adminUser) {
        setUser(adminUser)
